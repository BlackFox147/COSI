...
namespace Laba2_ImageIdentification.Cluster
{
    public class ClusterProcessing
    {
        ...
        public ClusterProcessing(int numClusters, IList<Shape> shapes)
        {
            rawData = new double[shapes.Count][];

            int i = 0;
            foreach (var shape in shapes)
            {
                rawData[i] = new [] { shape.Square, shape.Perimeter, shape.Compactness, shape.Elongation };
                i++;
            }
            ...
            //int[] clustering = Cluster(); 

        }

        public IList<Shape> Process()
        {
            int[] clustering = Cluster();
            ...
            foreach (var shape in shapes)
            {
                shape.NumberOfCluster = clustering[i];
                i++;
            }
            return shapes;
        }
